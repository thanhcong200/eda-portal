...
module.exports = createCoreService("api::ai-app.ai-app", ({ strapi }) => ({
    ...
    try {
      ...
      if (res.status === HttpStatusCode.Ok) {
        ...
        for (const item of Object.keys(data["tables_image"])) {
          ...
          // console.log(data["tables_image"][`${item}`]);
          // const path = await convertRGBToPng(
          //   data["tables_image"][`${item}`][0],
          //   fileName
          // );
          // filePaths[item] = path;
          const files = []
          ...
          for (const file of data["tables_image"][`${item}`]) {
            const fileName = `${entryApi.ai_app_document_id}${dayjs().get(
              "millisecond"
            )}${i}${j}.jpg`;
            convertBase64ToJPG(file);
            files.push(fileName)
            j++;
          } 
          i++;
  ...
  async like(ctx) {
    ...
    const isSuccess = await strapi.db
      .connection("ai_apps")
      .where({ document_id })
      .andWhereRaw("published_at IS NOT NULL")
      .increment({ like: quantity });
  ...
  async saveBookmark(ctx) {
    const { document_id } = ctx.params;
    const { is_save } = ctx.request.body;
    const userId = ctx.state.user.id;
    ...
    if (is_save === true) {
      let bookmark = await strapi.db
        .query("api::ai-app-bookmark.ai-app-bookmark")
        .findOne({
          where: {
      ...
      await strapi.db.query("api::ai-app-bookmark.ai-app-bookmark").delete({
        where: {
          ai_app: aiApp.id,
          user: userId,
        },
  ...
  async findAll(ctx) {
    ...
    const query = `
                    WITH bookmark_lnk AS ( SELECT ai.id as ai_app_id, COUNT(b.id) as quantity_saved
                      FROM ai_apps ai 
                      INNER JOIN ai_apps_ai_app_api_lnk api_lnk ON api_lnk.ai_app_id = ai.id
                      INNER JOIN ai_app_apis api ON api.id = api_lnk.ai_app_api_id AND api.published_at IS NOT NULL
                      ...
                      LEFT JOIN ai_app_bookmarks_ai_app_lnk b ON b.ai_app_id = ai.id
                      ...
                      SELECT ai.id as id, ai.document_id as document_id, ai.name as name, ai.short_desc, ai.scope as scope,
                       ai.po as po, ai.bu as bu, ai.impact as impact, f_image.url as url, f_image.formats as image,
                        ...
                        bookmark_lnk.quantity_saved as quantity_saved,
                      ...
                      LEFT JOIN bookmark_lnk ON bookmark_lnk.ai_app_id = ai.id
